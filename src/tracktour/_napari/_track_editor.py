...
class TrackEditor(Container):
    ...
    def _resolve_candidate_graph(self):
        ...
        if not self._candidate_graph:
            ...
            self._candidate_graph = FlowGraph(
                [(0, 0), seg_ims.shape[1:]],
                graph=sol_g,
                min_t=0,
                max_t=seg_ims.shape[0] - 1,
            )
        self._introduce_changes_to_candidate_graph()
        self._candidate_graph.solve()
        napari_graph_layer, seg = get_coloured_graph_labels(
            self._candidate_graph, self._seg_combo.value.data
        )
