...
class FlowGraph:
    ...
    def __init__(
        ...
        if coords is not None:
            ...
            if "z" in self._g.vs[0].attribute_names():
                self.spatial_cols.insert(0, "z")
            self._g.vs["coords"] = [
                tuple(v[col] for col in self.spatial_cols) for v in self._g.vs
            ]
    ...
    def _to_gurobi_model(self):
        ...
        for t in range(self.min_t, self.max_t + 1):
            ...
            for i, node in enumerate(t_nodes):
                ...
                m.addConstr(sum(node_in) == sum(node_out), f"migration_{i}")
                m.addConstr(sum(node_in) >= 1, f"forced_{i}")

                if not self.migration_only:
                    div_edge = flow.select("*", "*", "*", "division", node_id, "*")
                    m.addConstr(
                        sum(node_in) - sum(div_edge) >= sum(div_edge), f"div_{i}"
                    )
    ...
    def introduce_vertex(self, new_vid, t, coords, new_label, add_hyperedges=False):
        ...
        assert (
            v.index == new_vid
        ), f"New index was supposed to be {new_vid} but is {v.index}."
        if add_hyperedges:
            ...
            self._g.add_edge(
                self.appearance,
                v,
                var_name=f"e_a_{v['t']}.{v['pixel_value']}",
                cost=app,
                label=str(app)[:5],
            )
            # add v to target
            self._g.add_edge(
                v,
                self.target,
                var_name=f"e_{v['t']}.{v['pixel_value']}_t",
                cost=target,
                label=str(target)[:5],
            )
    ...
    def _clear_frame(self, source_t, dest_t):
        ...
        frame_mig_edges = self._g.es.select(
            lambda e: self._g.vs[e.source]["t"] == source_t
            and self._g.vs[e.target]["t"] == dest_t
        )
        self._g.delete_edges(frame_mig_edges)
        frame_div_edges = self._g.es.select(
            lambda e: self._g.vs[e.source]["is_division"]
            and self._g.vs[e.target]["t"] == source_t
        )
    ...
    def rebuild_frames(self, affected_ts):
        ...
        for t in affected_ts:
            tree, indices = self._build_tree_at_t(t)
            self._kdt_dict[t] = {"tree": tree, "indices": indices}
    ...
    def _get_frame_edges(self, source_t, dest_t):
        ...
        dest_distances, dest_indices = dest_tree.query(
            source_coords, k=k if k > 1 else [k]
        )
        frame_edges = []
        frame_var_names = []
        frame_labels = []
    ...
    def store_solution(self, opt_model):
        ...
        self._g.es["flow"] = 0
        self._g.es.select(list(v_dict.keys()))["flow"] = [
    ...
    def to_napari_graph(self):
        ...
        nx.set_node_attributes(
            nxg, nx.get_node_attributes(mig_subgraph, "track-id"), "track-id"
        )
        ...
        graph_layer = Graph(
            ...
            ndim=3,
            size=5,
            metadata={"nxg": nxg, "subgraph": mig_subgraph},
        )
        return graph_layer
...
def get_migration_subgraph(nx_sol: "nx.DiGraph"):
    ...
    for v in nx_sol.nodes:
        ...
        if not (
            v_info["is_appearance"]
            or v_info["is_target"]
            or v_info["is_division"]
            or v_info["is_source"]
        ):
            real_vs.append(v)
